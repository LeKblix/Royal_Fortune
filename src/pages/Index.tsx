...
const Index = () => {
  ...
  // Declare endGame function before it's used
  ...
  const ensureDeckHasCards = useCallback((needed: number): CardType[] => {
    if (deck.length >= needed) {
      return [...deck]; // Return a copy of the current deck
    }
    
    // Need to create a new shuffled deck
    return createDeck();
  }, [deck]);
  ...
  const discardSelectedCards = useCallback(() => {
    ...
    // Create a fresh deck for new cards
    const currentDeck = ensureDeckHasCards(selectedCount);
    
    // Create a new hand array
    const newHand = [...hand];
    
    // Replace each selected card with a new one
  ...
  const playSelectedCards = useCallback(() => {
    ...
    // Apply multipliers based on card combination
    switch (result.type) {
      case "Paire":
    ...
    // Record the hand in history regardless of points
    ...
    if (finalPoints > 0) {
      ...
      // Extract indices of selected cards for removal
      const selectedIndices = selectedCards
        .map((isSelected, index) => isSelected ? index : -1)
        .filter(index => index !== -1);

      // Create a new hand without the selected cards
      const newHand = [...hand];
      
      // Get fresh deck if needed
      ...
      // Update score and remaining hands after animation
      setTimeout(() => {
        ...
        // Apply the new hand and reset selection
        setHand(newHand);
        setSelectedCards(new Array(8).fill(false));
      }, 1600); // Matches timing with animation
    } else {
      toast({
  ...
  const handleUpgrade = (type: string) => {
    switch (type) {
      case 'multiply':
        setPointMultipliers(prev => ({ ...prev, pair: prev.pair * 3 }));
        break;
      case 'brelan':
        setPointMultipliers(prev => ({ ...prev, brelan: prev.brelan * 3 }));
        break;
      case 'straight':
        setPointMultipliers(prev => ({ ...prev, straight: prev.straight * 2 }));
        break;
      case 'flush':
        setPointMultipliers(prev => ({ ...prev, flush: (prev.flush || 1) * 2 }));
        break;
      case 'full':
        setPointMultipliers(prev => ({ ...prev, full: (prev.full || 1) * 3 }));
        break;
      case 'carre':
        setPointMultipliers(prev => ({ ...prev, carre: (prev.carre || 1) * 3 }));
        break;
      case 'straightFlush':
        setPointMultipliers(prev => ({ ...prev, straightFlush: (prev.straightFlush || 1) * 4 }));
        break;
      case 'doublePaire':
        setPointMultipliers(prev => ({ ...prev, doublePaire: (prev.doublePaire || 1) * 2 }));
        break;
    }
