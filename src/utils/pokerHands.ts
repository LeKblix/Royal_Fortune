...
export type PowerUpType = 'doublePoints' | 'extraCard' | 'wildcard' | 'reroll';
...
export const checkHand = (hand: Card[]) => {
  ...
  // Optimized calculate points function
  const calculatePoints = (type: string) => {
    ...
    switch (type) {
      ...
      case "Full": {
        ...
        return (threeValue * 3 + pairValue * 2) * 8; 
      }
      case "Flush": {
      ...
      case "Double Paire": {
        ...
        return (pairValues[0] * 2 + pairValues[1] * 2) * 4;
      }
      case "Paire": {
